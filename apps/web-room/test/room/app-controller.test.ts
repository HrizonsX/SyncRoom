...
} from "../../src/room/app-controller.js";
import {
  ProviderApiError,
  type ProviderApiClient,
} from "../../src/providers/provider-api-client.js";
import type { StorageLike, WebSocketLike } from "../../src/room/room-client.js";
...
test("ignores malformed room state messages before applying web-room state", () => {
  ...
  assert.equal(afterState.view, "joined");
  if (afterState.view !== "joined" || beforeState.view !== "joined") {
    throw new Error("expected joined state");
  }
  assert.equal(afterState.videoTitle, undefined);
  assert.deepEqual(afterState.diagnostics, beforeState.diagnostics);
});
...
test("shows unparseable link guidance when the generic extractor rejects the URL", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.equal(state.providerPicker?.status, "failed");
  assert.equal(state.providerPicker?.errorMessage, "该链接无法解析。");
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /稍后重试/);
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Bilibili/);
  assert.match(state.diagnostics.at(-1) ?? "", /extractor_unsupported_url/);
});

test("shows generic unsupported guidance for invalid parse input", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Bilibili/);
  assert.match(state.diagnostics.at(-1) ?? "", /unsupported_source/);
});

test("shows unparseable link guidance when generic extractor finds no playable video", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
      ...
      parse: async () => {
        throw new ProviderApiError(
          ...
          "Extractor request failed.",
          400,
          "extractor_no_playable_candidates",
        );
      },
    }),
  });
  ...
  await controller.parseBilibiliUrl({
    url: "https://www.pxb7.com/product/2259279867904646424/1",
    proxy: false,
    shared: false,
  });
...
test("shows unparseable link guidance when generic extractor only reports HTTP 422", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.equal(state.providerPicker?.errorMessage, "该链接无法解析。");
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /稍后重试/);
  assert.match(state.diagnostics.at(-1) ?? "", /extractor_http_422/);
});

test("shows service-unavailable guidance when generic extractor is unreachable", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Bilibili/);
  assert.match(state.diagnostics.at(-1) ?? "", /extractor_unavailable/);
});

test("keeps generic internal provider errors out of picker copy", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.equal(state.providerPicker?.status, "failed");
  assert.equal(state.providerPicker?.errorMessage, "解析失败，请稍后重试。");
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Internal/);
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Bilibili/);
  assert.match(state.diagnostics.at(-1) ?? "", /internal_error/);
});

test("keeps non-json provider transport failures out of picker copy", async () => {
  const { controller } = createJoinedHostController({
    providerApiClientFactory: () => ({
      startAuth: async () => ({
        providerId: "bilibili",
  ...
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Provider/);
  assert.doesNotMatch(state.providerPicker?.errorMessage ?? "", /Bilibili/);
  assert.match(state.diagnostics.at(-1) ?? "", /provider_request_failed/);
});
