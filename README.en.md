...
- Video sync
  ...
  - support Bilibili-specific pages and generic pages exposing a standard HTML5 `<video>`
...
- Admin panel
  - view overview, room list, room detail, runtime events, audit logs, and config summary
  - close rooms, clear shared video, kick members, and disconnect sessions
  - manage the IP blacklist manually or add a member IP to the blacklist from the room detail view
- In-page feedback
...
- app directory: `/opt/syncroom`
- service user: `syncroom`
- internal port: `8787`
...
Create `/etc/systemd/system/syncroom-room-node-a.service`:
...
Create `/etc/systemd/system/syncroom-global-admin.service`:
...
Create `/etc/syncroom/server.config.json` for shared non-secret settings:
...
Create `/etc/nginx/sites-available/syncroom.conf`:
