...
export type {
  WebRoomAuthPanelPhase,
  WebRoomAuthPanelState,
  WebRoomAuthMethod,
  WebRoomAuthStatus,
  WebRoomChatMessage,
  WebRoomConnectionState,
  WebRoomDanmakuMessage,
  WebRoomEntryState,
  WebRoomJoinedState,
  WebRoomMember,
  WebRoomPlaybackError,
  WebRoomPlaybackErrorStage,
  WebRoomProviderPickerItem,
  WebRoomProviderPickerState,
  WebRoomProviderPickerStatus,
...
function renderDanmakuLayer(state: WebRoomJoinedState): string {
  ...
  // 弹幕按真实时间衰减，而不是绑定视频 paused 状态；暂停视频时弹幕仍可继续飘完。
...
function renderPlayerProgressControl(
  ...
  if (source?.isLive) {
    // 直播没有可稳定 seek 的时间轴，渲染满格进度条以避免 media-time-range 循环跳动。
...
/**
 * 渲染播放器主体区域，集中处理播放权限、弹幕入口、加载状态和 media-chrome 控件。
 */
function renderPlayerSurface(state: WebRoomJoinedState): string {
  ...
  // gesturesdisabled 让视频主体点击遵守同一套播放权限，不能绕过控制栏按钮限制。
  return `
...
/**
 * 渲染平台授权弹框，按当前 Provider 状态展示授权、二维码、刷新和退出操作。
 */
...
function renderPlaybackSyncStrategyOptions(
  ...
  return `<div class="sync-strategy-options" role="group" aria-label="同步策略">
  ...
  const holdText =
    playbackSync.hold.active && waitingCount > 0
      ...
      : waitingCount > 0
        ...
        : "状态正常";
  const modeLabel =
    playbackSync.strategy === "smooth" ? "流畅优先" : "等人同步";
...
/**
 * 渲染房主的视频解析面板，承载链接解析、proxy/shared 策略和清晰度选择。
 */
  ...
  return `
    <section class="provider-picker-panel" data-panel="host-picker" data-picker-host="true" data-picker-status="${picker.status}">
      ...
      <div class="policy-row">
        <span class="policy-sync-strategy">
          ${renderPlaybackSyncStrategyOptions(getPlaybackSyncState(state))}
        </span>
        <span class="policy-options">
...
/**
 * 根据当前顶层状态选择入口页或房间页，是 web-room 的唯一 HTML 渲染出口。
 */
