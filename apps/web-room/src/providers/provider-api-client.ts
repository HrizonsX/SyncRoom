...
/**
 * 统一调用 Provider HTTP API，并把失败响应规整成前端可识别的 ProviderApiError。
 */
async function postJson<T>(
  ...
  const payload = await readProviderJsonResponse(response);
...
async function readProviderJsonResponse(response: Response): Promise<unknown> {
  ...
  try {
    ...
    // Provider API 经过本地服务或代理；故障时可能返回 HTML/纯文本。
    // 转成结构化错误，避免 UI 泄露 JSON 解析异常或后端原始内容。
...
/**
 * 创建 Provider API 客户端，统一封装授权、状态查询和视频解析请求。
 */
export function createProviderApiClient(
  serverUrl: string,
  fetchImpl: typeof fetch = fetch,
): ProviderApiClient {
  // 页面复用同一个服务地址建立 WebSocket 和 HTTP API；这里把 ws/wss
  // 规范化为 http/https，保证 Provider 请求跟随当前部署入口。
