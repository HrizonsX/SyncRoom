...
/**
 * 在 LiveKit 不可用时提供同样接口的运行时，让 UI 能展示明确错误而不是崩溃。
 */
...
/**
 * 创建基于 LiveKit 的浏览器语音运行时，封装房间连接、本地麦克风和远端音频轨道。
 */
...
/**
 * 绑定 LiveKit 房间事件，并转换成 web-room 自己的语音运行时事件。
 */
...
function handleTrackSubscribed(
  ...
  // LiveKit 返回的远端音频轨道需要挂到 DOM，浏览器才会真正播放。
  // 断开连接时 runtime 会统一移除这些隐藏 audio 元素。
