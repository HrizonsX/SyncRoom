...
} from "../../src/playback/playback-sync.js";
...
test("does not mark plain media pause events as user initiated", () => {
  const media = createMedia({
    currentTime: 42.5,
    playbackRate: 1,
    paused: true,
  ...
  const message = createPlaybackUpdateMessage({
    memberToken: "valid-member-token-123",
    actorId: "member-1",
  ...
  assert.equal(message.payload.playback.playState, "paused");
...
test("defers range-drag seek updates until pointer release", () => {
  const listeners = new Map<string, Set<() => void>>();
  const rangeListeners = new Map<string, Set<(event: Event) => void>>();
  const media = {
    ...createMedia({ currentTime: 24, paused: false }),
    addEventListener(type: string, listener: () => void) {
      const items = listeners.get(type) ?? new Set<() => void>();
  ...
  const timeRangeControl = {
    addEventListener(type: string, listener: (event: Event) => void) {
      const items =
        rangeListeners.get(type) ?? new Set<(event: Event) => void>();
      items.add(listener);
  ...
  rangeListeners
    .get("pointerdown")
    ?.forEach((listener) => listener(new Event("pointerdown")));
  media.currentTime = 36;
  rangeListeners
    .get("input")
    ?.forEach((listener) => listener(new Event("input")));
  listeners.get("seeked")?.forEach((listener) => listener());
  now = 7_000;
  ...
  rangeListeners
    .get("pointerup")
    ?.forEach((listener) => listener(new Event("pointerup")));
  now = 8_050;
  listeners.get("seeked")?.forEach((listener) => listener());
  binding.dispose();
...
test("holds media range seek requests while dragging to avoid intermediate media loads", () => {
  const listeners = new Map<string, Set<() => void>>();
  const rangeListeners = new Map<string, Set<(event: Event) => void>>();
  const media = {
    ...createMedia({ currentTime: 24, paused: false }),
  ...
  const dispatchRangeSeekRequest = (detail: number): boolean => {
    let stopped = false;
    const event = {
      detail,
      preventDefault() {},
    ...
    if (!stopped) {
      // Simulates media-controller applying media-chrome's request to the media element.
      media.currentTime = detail;
    }
    return stopped;
  };
  ...
  assert.equal(dispatchRangeSeekRequest(36), true);
  assert.equal(media.currentTime, 24);
  assert.equal(dispatchRangeSeekRequest(96), true);
  assert.equal(media.currentTime, 24);
  assert.deepEqual(dispatched, []);
  ...
  assert.equal(media.currentTime, 96);
  assert.deepEqual(dispatched, [
    {
      type: "playback:update",
      payload: {
...
test("dispatches rapid media-chrome play toggle requests as explicit last-intent controls", () => {
  const listeners = new Map<string, Set<() => void>>();
  ...
  toggleListeners.get("mediaplayrequest")?.forEach((listener) => listener());
  toggleListeners.get("mediapauserequest")?.forEach((listener) => listener());
...
test("suppresses stale opposite media echo after rapid media-chrome play toggles", () => {
  const listeners = new Map<string, Set<() => void>>();
  ...
  const playToggleControl = {
    addEventListener(type: string, listener: () => void) {
      const items = requestListeners.get(type) ?? new Set<() => void>();
      items.add(listener);
  ...
  requestListeners.get("mediaplayrequest")?.forEach((listener) => listener());
  requestListeners.get("mediapauserequest")?.forEach((listener) => listener());
  media.paused = false;
  listeners.get("play")?.forEach((listener) => listener());
...
test("dispatches media-chrome play requests as explicit controls", () => {
  const listeners = new Map<string, Set<() => void>>();
  const requestListeners = new Map<string, Set<() => void>>();
  const media = {
    ...createMedia({ paused: true }),
  ...
  assert.deepEqual(
    dispatched.map((message) => {
      ...
      return [playback.playState, playback.syncIntent, playback.userInitiated];
    }),
    [
      ["playing", "explicit-play", true],
      ["paused", "explicit-pause", true],
    ],
  );
  binding.dispose();
});
