...
import { webRoomThemeTokens } from "../../src/ui/theme.js";
...
test("renders host playback sync strategy controls", () => {
  ...
  assert.doesNotMatch(html, /data-panel="playback-sync"/);
  const policyRowStart = html.indexOf('class="policy-row"');
  const policyRowHtml = html.slice(
    policyRowStart,
    html.indexOf('class="provider-result-list"', policyRowStart),
  );
  assert.match(policyRowHtml, /data-action="set-playback-sync-strategy"/);
  assert.match(policyRowHtml, /data-sync-strategy="smooth"/);
  assert.match(policyRowHtml, /data-sync-strategy="wait"/);
  assert.match(policyRowHtml, /name="providerProxy"/);
  assert.match(policyRowHtml, /name="providerShared"/);
  assert.ok(
    policyRowHtml.indexOf('data-sync-strategy="smooth"') <
      policyRowHtml.indexOf('name="providerProxy"'),
  );
});
